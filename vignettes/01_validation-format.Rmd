...
vignette: >
  %\VignetteIndexEntry{Validation format}
...
- Describe the format of __exposure__.

## Design process

The preferred local data structure in `tidyverse` is [tibble](http://tibble.tidyverse.org): "A modern re-imagining of the data frame", on which its implementation is based. That is why `ruler` takes data frames as preferred format for validated data. However the initial goal is to use tibbles in creation of validation result format as much as possible.
...
The validation of data units can be done with the functions described above. Their application to some data unit can give answers to multiple questions (differing only in their interpretation). That is why by design __rules__ (functions that answer one certain question) are combined in __rule packs__ (functions that answer multiple questions for one data unit).

Application of rule pack to data is connected with several points:

- Rule packs should have unique __names__ to be used as reference to it.
- By the same reason rules should have names. However uniqueness is necessary only within corresponding rule pack.
- Output of rule packs for different data units differ in their structure. Therefore rule packs should have __types__.
- During the actual validation the most part of results normally indicates obedience to rules. This can cause dragging many redundant information. `ruler` has option of __removing obeyers__ from validation result.
