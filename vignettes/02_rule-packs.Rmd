...
This vignette describes and explains logic behing common ways of creating rule packs.
...
Some of `ruler`'s functionality is powered by the [keyholder](http://github.com/echasnovski/keyholder) package. It is highly recommended to use its supported functions during rule construction. All one- and two-table `dplyr` verbs applied to local data frames are supported and considered the most appropriate way to create rule packs.

As described in vignette about validation format it is necessary for rule pack to have __type__ because outputs for different data units have different structure. For this reason `ruler` has family of `*_packs()` constructors (where `*` stands for the name of data unit):

- They take functions (in pure form or inside unnamed list at any depth) defining packs as arguments. It is recommended to name those arguments with future pack names. If no name supplied it will be imputed during exposure.
...
replaced with `.`. To indicate that this function is a rule pack data unit 'data' it should be wrapped with `data_packs()`.

The next code creates a list `my_data_packs` with one data rule pack with name `my_data_pack_1`. That rule pack defines defines rules with names `nrow_low`, `nrow_high`, `ncol`.
...
- Names of non-grouping define as __rule names__.
...
- Pack's output is a grouped tibble. It doesn't matter because during exposure this output is `ungroup`ed.
...
In general it is hard to automatically separate output's column names into 'validated column name' and 'rule name' because default separator `_` is a commonly used one. For this reason `ruler` has function `rules()` which wraps around `funs()` with the following functionality:
...
- Add rare prefix `._.` (Morse code for 'R') to rule names. __Note__ that one can change this prefix with `.prefix` argument.
...
- A nameless (will be imputed as `col_pack..2`) column rule pack which checks obedience of column `vs` to some (will be imputed as `rule..1`) rule. __Note__ the use of named argument in `vars(vs = "vs")`. This is the current way in `dplyr`'s scoped `summarise` and `mutate` to force using both column and function names in output's column name.
...
Pipeline like the one above is quite common: compute some value based on all rows and then validate only some of them. However in the validation report column `id` should represent the row index _in the original data frame_. But this information is missing after applying of `slice()`.

This problem is solved by using [keyholder](https://github.com/echasnovski/keyholder) package. Its main purpose is to track information about rows during data frame modifications. While exposing pack is applied to the __keyed__ version of input data with key equals to row index. __Note__ that to use this feature one should create rule packs using composition of functions supported by `keyholder`.
...
The next code creates a list with one cell pack `my_cell_pack_1`. It checks cell of every integer-like column in rows 20-24. The rule `is_common` checks the values for not being an outlier within column.
