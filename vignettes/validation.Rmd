...
- It takes data as a first argument and then rule packs (in pure form or inside list at any depth) of interest.
- All rule packs are actually applied to __keyed__ version of data (see [keyholder](http://github.com/echasnovski/keyholder)) for reasons stated in "Rule Packs" vignette. If input has keys they are removed and _id key_ is created.
- It is guaranteed that its output is equivalent to the input data frame: only attribute `exposure` might change. If input has `exposure` attached to it then the new one is binded with it.
...
By default (if not set manually) `expose()` guesses which type of pack function represents. This is useful for interactive experiments. Guess is based on features of pack's output structures (see `?expose` for more details).
